...
## Phase 4 — Pixel-Art Steam / Smoke Spec

> [!IMPORTANT]
> Steam and smoke must match the **Minimal Industrial Pixel-RPG aesthetic** — use the same 2D pixel-art sprite style as the rest of the game world, **not** procedural circles. Sprites are pre-generated with **nano-banana** before the build step and loaded as textures into PixiJS.

### Reference Image

The uploaded reference shows 6 cloud puff frames:
- Row 1: Small tight puff → Medium billow → Large billowed cloud (growth)
- Row 2: Dense full cloud → Dispersing with pixel scatter → Final dissipation fragments

This is the exact animation cycle each machine vent emits and loops over.

### Sprite Generation (nano-banana)

Generate **three separate sprite sheets** — one per machine condition tier — using the `generate_image` tool before building the particle system.

#### Tier 1 — Good (Normal Operation)
| Property | Value |
|----------|-------|
| Color | Cool pale silver-white (`#D8D8D8` → `#EFEFEF`) |
| Frame count | 6 frames |
| Frame size | 32×32px each |
| Sheet layout | 6 frames in a horizontal strip (192×32px total) |
| Prompt keywords | `pixel art, steam puff, small wispy cloud, silver-white, warm factory, top-down game sprite, 6 frames, transparent background` |
| Opacity in canvas | 0.35 – 0.55 |
| Emission rate | 1 new emitter every ~3 000ms |
| Scale | 0.5 – 0.7× |

#### Tier 2 — Watch (Degrading)
| Property | Value |
|----------|-------|
| Color | Warm grey with amber tint (`#C0B090` → `#A08060`) |
| Frame count | 6 frames |
| Frame size | 48×48px each |
| Sheet layout | 6 frames horizontal strip (288×48px total) |
| Prompt keywords | `pixel art, smoke puff, medium billow, warm grey amber tint, industrial machinery, 6 frames, transparent background` |
| Opacity in canvas | 0.55 – 0.75 |
| Emission rate | 1 new emitter every ~1 200ms |
| Scale | 0.7 – 1.0× |

#### Tier 3 — Critical (Severe)
| Property | Value |
|----------|-------|
| Color | Dark charcoal-brown with reddish base (`#6A5040` → `#4A3428`) |
| Frame count | 6 frames |
| Frame size | 64×64px each |
| Sheet layout | 6 frames horizontal strip (384×64px total) |
| Prompt keywords | `pixel art, thick dark smoke, heavy billow, charcoal brown, ominous factory, dispersing pixel fragments, 6 frames, transparent background` |
| Opacity in canvas | 0.75 – 0.95 |
| Emission rate | 1 new emitter every ~400ms |
| Scale | 1.0 – 1.4× |

### Animation Behavior

- Each active emitter spawns an `AnimatedSprite` at the machine's vent point (top-center of machine box).
- The sprite plays frames 0→5 once (not looping), then destroys itself.
- Frame speed: `animationSpeed = 0.12` (Good) / `0.18` (Watch) / `0.24` (Critical).
- **Rise drift:** While frame is playing, apply `y -= 0.4 * deltaMS * 0.06` so the puff floats up ~15–25px over its lifetime.
- **Fade-out:** `alpha` decreases linearly with frame progress (`alpha = 1 - (currentFrame / 5) * 0.8`), so it starts fully opaque and ends near-transparent.
- **Horizontal sway:** `x += Math.sin(time * 0.003 + emitterSeed) * 0.2` for a gentle lateral drift.
- Multiple overlapping emitters are allowed (pooled, max 8 per machine to keep GPU load low).

### PixiJS Integration

```ts
// Assets pre-loaded at app startup
import smokeGoodSheet from '../assets/smoke_good.png';
import smokeWatchSheet from '../assets/smoke_watch.png';
import smokeCriticalSheet from '../assets/smoke_critical.png';

// Per-tier Texture arrays carved from horizontal strips
const smokeTextures: Record<MachineStatus, Texture[]> = {
  good:     carveStrip(smokeGoodSheet,     32, 6),
  watch:    carveStrip(smokeWatchSheet,    48, 6),
  critical: carveStrip(smokeCriticalSheet, 64, 6),
  isolated: [], // no smoke for isolated machines
};

function carveStrip(sheet: Texture, frameSize: number, count: number): Texture[] {
  return Array.from({ length: count }, (_, i) =>
    new Texture({ source: sheet.source, frame: new Rectangle(i * frameSize, 0, frameSize, frameSize) })
  );
}
```
...
```
src/assets/smoke_good.png      ← Generated by nano-banana (192×32px)
src/assets/smoke_watch.png     ← Generated by nano-banana (288×48px)
src/assets/smoke_critical.png  ← Generated by nano-banana (384×64px)
src/canvas/SmokeSystem.tsx     ← Replaces current procedural ParticleSystem.tsx
```

### What Changes vs Current Implementation

The current `ParticleSystem.tsx` uses procedural circles drawn via `Graphics`. This is **replaced** in Phase 4 (revised) with:
- Sprite-based `AnimatedSprite` puffs from the generated sheets.
- Each tier uses the correct sheet/textures.
- The `SmokeSystem` component manages an array of active `AnimatedSprite` instances via a `useTick` loop.
- Procedural circle fallback remains as a `SMOKE_FALLBACK` flag in case assets fail to load.
...
### Phase 4 — Pixel-Art Smoke System + Aging (Hour 6-7)
- **Pre-build:** Generate 3 smoke sprite sheets with nano-banana (Good/Watch/Critical tiers, 6 frames each, horizontal strips)
- Load textures at startup; `carveStrip()` slices each sheet into per-frame `Texture[]`
- Replace procedural `ParticleSystem.tsx` circles with `SmokeSystem.tsx` using `AnimatedSprite` puffs
- Smoke color and intensity scale with machine status: pale silver (Good) → warm amber-grey (Watch) → dark charcoal-brown (Critical)
- Each puff plays 6 frames once, rises ~15-25px, fades out, then destroys itself (pooled, max 8 per machine)
- Machine aging visual progression: rust overlay at 50/80/90% age pct, corner rust pixels, darkening tints
- Age badge (`! X.Xy`) rendered above machines at ≥90% age
- **Checkpoint: Machines breathe pixel-art smoke scaled by health, old machines look visibly worn**
...
To ensure efficient, high-quality development, we will leverage the following installed skills:

- **web-artifacts-builder**: Used to initialize the React project via `bash scripts/init-artifact.sh` and eventually package the final dashboard into a single shareable bundle with `bash scripts/bundle-artifact.sh`.
- **react-components**: Used to construct and refine complex, interactive React UI components (like `MachinePanel`, `TopBar`, and `AgentLogsDrawer`) adhering to modern frontend best practices.
- **design-md**: Used to maintain and enforce our *Minimal Industrial Pixel-RPG* design system constraints—ensuring colors, typography, and spacing remain consistent.
- **enhance-prompt**: Employed to write highly specific, high-fidelity prompts for generating our `nano-banana` pixel-art assets (factory maps, machine variants, and worker sprites).
- **find-skills**: Utilized during development to discover and incorporate additional necessary capabilities for obscure rendering, state management, or debugging scenarios.
- **stitch-loop**: Employed for rapid, state-driven UI iteration when refining complex layouts, slide-in panels, and interactive elements.
- **web-app**: Used to bootstrap, configure, and maintain the overarching Vite + React + TypeScript web application environment.
